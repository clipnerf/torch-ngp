...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None):
    ...
    if N > 0:
        ...
        if error_map is None:
            ...
            sx, sy = H / 128, W / 128
            inds_x = (inds_x * sx +
                      torch.rand(B, N, device=device) * sx).long().clamp(max=H -
                                                                         1)
            inds_y = (inds_y * sy +
                      torch.rand(B, N, device=device) * sy).long().clamp(max=W -
                                                                         1)
    ...
    def test(self, loader, save_path=None, name=None):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                preds_depth = preds_depth / np.max(np.max(preds_depth, axis=-2),
                                                   axis=-2)[:, None, None, :]
                # cv2.imwrite(
                #     path,
                #     cv2.cvtColor((preds[0].detach().cpu().numpy() * 255).astype(
                #         np.uint8), cv2.COLOR_RGB2BGR))
                # cv2.imwrite(path_depth, (preds_depth[0].detach().cpu().numpy() *
                #                          255).astype(np.uint8))
                # if write_video:
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    # cv2.imwrite(
                    #     save_path_depth,
                    #     cv2.cvtColor((cm.inferno(
                    #         normalized_depth[0].detach().cpu().numpy()) *
                    #                   255).astype(np.uint8), cv2.COLOR_RGB2BGR))
